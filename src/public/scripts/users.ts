...
const userOptions = document.getElementById("Options") as HTMLSelectElement;
...
// on search of userId 
formId.addEventListener('submit',async (e:Event)=>{
    e.preventDefault();
...
async function optionsFunction1(e:Event): Promise <void>{
...
function deleteOrUpdateUser(option: "Delete" | "Update"):void{
...
async function update1(id:string){
    const user = await fetch(`/lib/users/${id}`);
